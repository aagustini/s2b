...
namespace Lab11_LinqtoObjects
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            #region  Console.WriteLine(">>> Pessoas agrupadas pelo mes de nascimento, ordenado");
            var r27 = (from p in pessoas
                      group p by p.DataNascimento.Month).ToList();


            r27.ForEach(e =>
            {
                Console.WriteLine("chave: " + e.Key);
                 e.ToList().ForEach(p=> Console.WriteLine("\tnome: " + p.Nome)); 
            });
            foreach (var g in r27.OrderBy(p=>p.Key))
            {

                Console.WriteLine("Mes: " + (g.Key) + 
                                  " quantidade: " + g.Count());

                foreach (var p in g.OrderBy(p=>p.Nome))
